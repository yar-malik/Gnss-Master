...
/* decode type 1057,1060 message header --------------------------------------*/
                           ...
                           int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    trace(4,"decode_head1057: time=%s nsat=%d sync=%d provid=%d solid=%d\n",
          time_str(rtcm->time,1),nsat,*sync,provid,solid);
    
    *hsize=i;
    return nsat;
}
/* decode type 1058,1059,1061,1062 message header ----------------------------*/
static int decode_head1058(rtcm_t *rtcm, int *sync, double *udint, int *hsize)
{
    double tow;
    int i=24+12,nsat,udi,provid=0,solid=0;
    
    if (i+55>rtcm->len*8) return -1;
    tow   =getbitu(rtcm->buff,i,20); i+=20;
    udi   =getbitu(rtcm->buff,i, 4); i+= 4;
    *sync =getbitu(rtcm->buff,i, 1); i+= 1+4;
    provid=getbitu(rtcm->buff,i,16); i+=16;   /* provider id */
    solid =getbitu(rtcm->buff,i, 4); i+= 4;   /* solution id */
    nsat  =getbitu(rtcm->buff,i, 6); i+= 6;
    *udint=ssrudint[udi];
    ...
    trace(4,"decode_head1058: time=%s nsat=%d sync=%d provid=%d solid=%d\n",
          time_str(rtcm->time,1),nsat,*sync,provid,solid);
...
static int decode_type1057(rtcm_t *rtcm)
{
    ...
    int i,j,k,sync,nsat,prn,sat,iode,refd=0;
    
    if ((nsat=decode_head1057(rtcm,&sync,&udint,&refd,&i))<0) {
...
{
    double udint,dclk[3];
    int i,j,k,sync,nsat,prn,sat,rsv=0;
    
#ifndef SSR_DRAFT
    if ((nsat=decode_head1058(rtcm,&sync,&udint,&i))<0) {
#else
    if ((nsat=decode_head1057(rtcm,&sync,&udint,&rsv,&i))<0) {
#endif
...
static int decode_type1059(rtcm_t *rtcm)
{
    ...
    int i,j,k,mode,sync,nsat,prn,sat,nbias,rsv=0;
...
static int decode_type1060(rtcm_t *rtcm)
{
    ...
    int i,j,k,nsat,sync,prn,sat,iode,refd=0;
...
{
    double udint;
    int i,j,nsat,sync,prn,sat,ura,rsv=0;
...
{
    double udint,hrclk;
    int i,j,nsat,sync,prn,sat,rsv=0;
...
/* decode type 1063,1066 message header --------------------------------------*/
                           ...
                           int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    trace(4,"decode_head1063: time=%s nsat=%d sync=%d provid=%d solid=%d\n",
          time_str(rtcm->time,1),nsat,*sync,provid,solid);
    
    *hsize=i;
    return nsat;
}
/* decode type 1064,1065,1067,1068 message header ----------------------------*/
static int decode_head1064(rtcm_t *rtcm, int *sync, double *udint, int *hsize)
{
    double tod;
    int i=24+12,nsat,udi,provid=0,solid=0;
    
    if (i+52>rtcm->len*8) return -1;
    tod   =getbitu(rtcm->buff,i,17); i+=17;
    udi   =getbitu(rtcm->buff,i, 4); i+= 4;
    *sync =getbitu(rtcm->buff,i, 1); i+= 1+4;
    provid=getbitu(rtcm->buff,i,16); i+=16;
    ...
    *udint=ssrudint[udi];
    ...
    trace(4,"decode_head1064: time=%s nsat=%d sync=%d provid=%d solid=%d\n",
          time_str(rtcm->time,1),nsat,*sync,provid,solid);
...
static int decode_type1063(rtcm_t *rtcm)
{
    ...
    if ((nsat=decode_head1063(rtcm,&sync,&udint,&refd,&i))<0) {
...
#ifndef SSR_DRAFT
    if ((nsat=decode_head1064(rtcm,&sync,&udint,&i))<0) {
#else
    if ((nsat=decode_head1063(rtcm,&sync,&udint,&rsv,&i))<0) {
#endif
